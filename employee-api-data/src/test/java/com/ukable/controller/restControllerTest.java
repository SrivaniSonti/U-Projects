...
public class restControllerTest {
	...
	public void saveDepartmentTest() throws JsonProcessingException, Exception {
		...
		MvcResult result = mockMvc.perform(
				post("/departments").contentType(APPLICATION_JSON_VALUE).content(MAPPER.writeValueAsString(department)))
	...
	public void createEmployeeTest() throws JsonProcessingException, Exception {
		...
		MvcResult result = mockMvc.perform(post("/departments/{department_Id}/employees", "1")
	...
	public void saveEmployeeTest() throws JsonProcessingException, Exception {
		Employee employee = new Employee("1", "srivani", "123456", new Department("1", "Testing"));
		mockMvc.perform(put("/departments/{department_Id}/employees/{employee_id}", "1", "1")
				.contentType(APPLICATION_JSON_VALUE).content(MAPPER.writeValueAsString(employee)))
				.andExpect(status().isOk()).andReturn();
	}
	...
	public void getEmployees() throws JsonProcessingException, Exception {
		MvcResult result = mockMvc.perform(get("/employees").contentType(APPLICATION_JSON_VALUE))
				.andExpect(status().isOk()).andReturn();
	...
	public void deleteEmployee() throws Exception {
		MvcResult result = mockMvc.perform(delete("/departments/{department_Id}/employees/{employee_id}","1","1").contentType(APPLICATION_JSON_VALUE))
				.andExpect(status().isOk()).andReturn();
	}
